...
\title{脆弱性情報アップストリームの不安定性の実測: \\ 履歴管理基盤上の公開前検査による116日間の全数運用調査}
\etitle{Measuring Upstream Instability of Vulnerability Information: A 116-Day Exhaustive Study of Pre-Publication Inspection on a History-Managed Database Pipeline}
...
116日間・945 runの全数調査により，上流フィードの不安定性を69件の独立イベントとして分類・定量化した．
...
\textbf{全数調査}:
本番導入(2026-04-23)から08-17までの116日間，2系統(安定版 / nightly)の全945 runを対象に，runログ，PR，promote履歴をGitHub APIで全数収集した．
...
945 run中，失敗は476 run．うち420 runが検査のステップで失敗した(閾値超過418+検査内インフラ障害2)．
独立イベントは69 source-episode，事例カタログとしては約41イベント(複数ソースの同時多発を1事例に束ねた粒度)である．
...
判定 & 件数 & 代表例 \\
\hline
上流由来(a)正当な変更 & 約31/41 & 月次パッチ等 \\
上流由来(b)一時的障害(\textbf{公開阻止}) & 2 & CVRF消失$\times$2 \\
上流由来(c)恒久的品質イベント & 2 & errata再編 \\
抽出器由来・コードバグ(\textbf{公開阻止}) & 1 & 非決定的出力 \\
抽出器由来・意図的変更 & 3 & 検知の補完・拡張 \\
取得設定由来(orchestration) & 2 & seed追加，新ソース \\
ビルダ由来 & 0 & (精査したが無罪) \\
\hline
\end{tabular}
\end{table}

観測期間の後半では，前半に存在しなかった2つの原因カテゴリが現れた．
ひとつは\textbf{取得設定由来}である．
我々自身が取得対象のseedリストを拡張した直後(2026-08-05から08-06のKB seed登録)にmicrosoft/microsoft-msucが211.3\%で発動し，新規データソースを追加した直後(08-10)にはmicrosoft/microsoft-servicingがbaseline不在のため100.0\%で発動した．
上流もコードも変わっておらず，「何を取りに行くか」の設定変更が原因である．
もうひとつは\textbf{抽出器由来の意図的変更}の再来で，2012年から2022年の履歴的アドバイザリに検知を補完する抽出器変更(検知を持つファイルが643件から1,065件に増加)のマージ直後に，cpe\_fortinet/fortinet-cvrfが135.5\%で発動した．
いずれも上流の異常ではなく我々の変更であり，帰属手順にvuls-data-db側のコミット走査(\ref{sec:method}章の手順3)を加えることで初めて分離できた．
...
116日間の手動promoteは92 unique digestあり，その\textbf{100\%が検査FAIL runの候補digestと一致}した．
...
観測された最大変動率は3433.3\%(新規リリースfedora:46)，428.3\%(AlmaLinux再キュレーション)，305.9\%(CPE系)に達する一方，日常変動は5から20\%帯に収まり，両者は概ね桁で分離する．
...
\ref{sec:prevented}章で述べた「分解能不足で判定を誤りかけた」事例と同型の希釈は，後半の窓では観測されていない．
...
fail sequenceの持続時間(中央値10h / p90 48h)は検査の検出性能ではなくオペレータの応答時間の分布を測っており，通知や承認フローの自動化余地を見積もる直接の根拠となる．
...
脆弱性DBの公開CIに公開前検査を実装し，116日間・945 runの全数調査を通じて，脆弱性情報の上流ソースの不安定性を69件の独立イベントとして定量観測した．
...
(5)観測を116日へ延長した後半では，上流でもコードでもなく「何を取りに行くか」の設定変更を原因とするイベントが現れ，帰属手順に取得設定の走査を加える必要が生じた．運用が続く限り原因カテゴリは増え，帰属手順もそれに追随して更新すべきものである．
(6)生成物を出力レベルで検証する検査は，上流だけでなく自らの開発プロセスに対する最終防衛線としても機能し，テストとレビューをすり抜ける意味的バグの捕捉や，破壊的変更の影響範囲の定量確認に実際に寄与した．

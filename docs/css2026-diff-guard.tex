...
\begin{table}[tb]
\caption{episodeの代表判定の分布(計69件)．複数原因を含むepisodeは公開可否を左右した原因で代表させた．系列推定の4件は開発版で先行したCPE関連変更群への帰属で，工程(抽出器かビルダか)は未確定．他の自前由来6件は原因のコミットまで特定済み．上流由来(a)はfedora:46のような小規模baseline起因の構造的FAILを含む．}

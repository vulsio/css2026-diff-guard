...
errataフィードが1週間に複数回再編成され，アドバイザリ数が279から一時60まで減少，削除されたアドバイザリが参照するCVEの87\%(376件)がフィードから完全消失，残ったアドバイザリでもkernel更新の検知条件が73から2パッケージへ縮退した(最大変動率428.3\%)．
過去バージョン系列に対する大量のfalse negativeに直結する縮退であり，復旧作業の過程で縮退DBは数時間公開に達したが，当該ソースの取り込みを縮退前のコミットにピン留めしたビルドを同日中にpromoteして巻き戻し，以後の流入を停止した．
その後，自らの取得履歴のスナップショットから欠落分のアドバイザリをfetch時に復元する機構(上流のHTML indexに存在するのにフィードから消えたものだけを復元する)を組み込んだ上でアンピンした．
ピン留めも履歴からの復元も，取り込みが履歴管理されているから可能な操作である(縮退の原因は\ref{sec:integrity}節)．
...
実際，期間中に唯一捕捉されたコード起因の異常(表\ref{tab:attribution}の抽出器由来バグ)は，型の取り違えでソート処理が欠落し出力順序が非決定になるもので，コンパイルもテストも通過する一方，rawの変更が266ファイルなのにextractedが23,163ファイル動く構造差分(drift 274.5\%)として顕在化し，公開阻止と修正PRに帰結した．

...
生き残ったアドバイザリも，kernel更新の検知条件が73から2パッケージへ縮退した．
フィード，OSV，HTMLの三者が一致したことから，これは配信障害ではなく，フィードが現行バージョンの情報しか保持しないという上流仕様の顕在化であると判明した(AlmaLinuxのissue trackerでも既知の問題である\footnote{\url{https://bugs.almalinux.org/view.php?id=644}})．
上流が仕様どおりに動いていても，過去バージョンの範囲情報を必要とする脆弱性検知にとってはデータの欠損として働く例である．
対応として，当該ソースの取り込みをコミット単位でピン留めし，代替ソース(OVAL)の抽出器を整備してから計画的にアンピンした．
...
運用は検査自身の弱点も露呈させた．
(1)DBチェックのKB比較はKBキーの集合しか見ておらず，CVRF消失(\ref{sec:prevented}節の事例1)ではキー集合が不変のままCVEエントリが大量消失したため0\%を返した(「0\% $\neq$ 変化なし」．このとき公開を止めたのはDnのみである)．

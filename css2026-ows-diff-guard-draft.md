...
- [ ] 本文中の em-dash(「〜 — 〜」挿入構文)を句読点・接続詞に書き換える(概要は対応済み。§1, §4, §5, §6 に残存)
...
**概要**: 脆弱性スキャナが依存する脆弱性データベースは、ディストリビュータの advisory フィードや NVD をはじめとする多数の上流データソースを集約して構築される。これらの上流は必ずしも安定していない。月次データの丸ごと消失と復活、公開済み advisory の遡及的な縮退、アナウンスのない大規模な再生成といった変化が実際に起きている。本研究では、この上流の不安定性を本番運用の中で定量観測した結果を報告する。観測装置として、OSS 脆弱性スキャナ Vuls の DB 公開 CI に公開前差分ゲートを実装した。ゲートは公開候補 DB を直前の公開版と比較し、スキャナの検知結果と DB 内の検知条件構造という 2 観点の変動率が閾値を超えた場合、公開をブロックして人間の判断に委ねる。82 日間・671 run の全数調査の結果、ゲートは 50 件の独立イベントで発動し、その全てについて原因を上流由来、抽出器由来、ビルダ由来のいずれかに帰属できた。この帰属は、昨年報告した収集・変換・構築の全工程をバージョン管理する履歴管理基盤によって可能になった。また、日常的な変動(5〜20%)と構造的イベント(100% 超)は変動率で桁が分離しており、単純な閾値ゲートが実用になることを確認した。運用期間中、ゲートは欠損 DB の公開を 3 回阻止した。

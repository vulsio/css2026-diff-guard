...
タイトル代案(em-dash 不使用・「公開前検査」表記で統一):
- 「脆弱性情報の上流はどれほど動くか: 公開前検査による 82 日間の定量観測」
- 「公開前検査で観測した脆弱性フィードの不安定性と公開品質管理の実践」
...
- [ ] 著者・所属・順序の確認(css.tex に仮で 篠原→中岡。要確認 TODO コメント入り)
- [ ] **研究倫理セクションのレビュー** — TeX 化時に新規起草したもの(css.tex 末尾)。内容の妥当性を要確認
- [ ] 観測期間を投稿時点まで延長するか決める — 現本文は 82 日 / 671 run / 50 episode(〜07-14)固定。延長するなら `stats.py` を `guard-failures.tsv` に再実行して全数値を更新
- [ ] 図 4 枚の作成と css.tex の \fbox プレースホルダ差し替え(図 1 = TikZ、図 2〜4 = matplotlib で PDF。スクリプト管理し期間延長時に再実行で追従)
  - fig:arch パイプライン全体図 / fig:benchmark 導入前ベンチマーク時系列(データ: diff-guard.md の 35 ペア表)/ fig:timeline 発動ガントチャート(データ: guard-failures.tsv)/ fig:histogram 変動率ヒストグラム(同)
  - 図を足すとページが増えるため 8 ページ上限との兼ね合いを図作成時に確認
...
- [x] 用語統一: ゲート → 公開前検査 / 検査(タイトル・見出し含む)
- [x] advisory → アドバイザリ。fixture / baseline / target / promote / grooming / override は英字のまま(コマンドや運用用語として扱う)
- [x] 「抽出器」「ビルダ」を §2.1 で初出定義
- [x] 参考文献を refs.bib 化、仮キー → \cite
- [x] 研究倫理セクション新設(※内容レビューは上記 TODO)
- 注: run ID / digest は本文に元々含めていないため脚注化は不要だった

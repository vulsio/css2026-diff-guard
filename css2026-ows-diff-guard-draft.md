...
**概要**: 脆弱性データベースは多数の上流データソースを集約して構築されるが、上流は必ずしも安定せず、データの消失や公開済み advisory の遡及的な書き換えが現実に起きている。本研究では、この不安定性を定量観測するため、OSS 脆弱性スキャナ Vuls の DB 公開 CI に、公開候補を直前の公開版と検知結果・検知条件構造の 2 観点で比較し、変動率が閾値を超えると公開をブロックする差分ゲートを実装した。82 日間の本番運用の全数調査では 50 件の独立イベントで発動し、昨年報告した全工程の履歴管理基盤により、全てを上流・抽出器・ビルダのいずれか由来に帰属できた。日常変動と構造的イベントは変動率で桁が分離し閾値ゲートが実用になること、またゲートが欠損 DB の公開を 3 回阻止したことを示す。

...
- 原因帰属の証拠水準(§4 に明記): 自前由来 10 episode のうち 6 件は原因コミット/PR で直接立証、4 件は開発版 CPE 変更系列への帰属で変換工程かDB構築工程か未確定。上流由来はDB構築コード、収集コード、変換コード、収集対象設定をコミット履歴で除外する消去法(raw smoking gun は代表事例のみ)。episode 別の対応は data/04-episode-verdicts.md

...
- 原因帰属の証拠水準(§4 に明記): 自前由来 10 episode のうち 6 件は原因コミット/PR で直接立証、4 件は開発版 CPE 変更系列への帰属で工程未確定。上流由来はビルダ・抽出器・取得設定をコミット履歴で除外する消去法(raw smoking gun は代表事例のみ)。episode 別の対応は data/04-episode-verdicts.md
...
全エントリ 2026-07-29 に出典ページで実在確認済み(lin26 は 2026-08-19 に頁・著者順を刊行版で確定)。現在未引用のエントリ(nguyen13, ruohonen19, wunder24, torres19, lamb22, newman22, samuel10, slsa)も bib と本リストに残置。refs.bib に URL の無い学術文献の検証先:

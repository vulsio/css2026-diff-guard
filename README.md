...
- 用語: 「公開前検査(diff guard)」を §1 で定義し、以後「検査」または「diff guard」。「ゲート」は使わない。工程名は「収集(fetch)」「変換(extract)」「DB構築(db build)」、実装は「収集コード」「変換コード」「DB構築コード」とする。収集起点となるIDやURLと対象ソースを定める設定は「収集対象設定」とし、「seed」「抽出器」「ビルダ」「取得設定」は使わない

...
**除外** = DB構築コード/変換コード/収集対象設定のコミット走査による消去法(raw smoking gun 未確認)、
**系列** = A-8 開発版の変換コード変更系列への帰属(個別 triage 無し)、**事後分類** = Phase 1〜2 期の run 表からの遡及分類。

集計: 上流(a) 55 / 上流(b) 2 / 上流(c) 2 / 変換工程(バグ) 1 / 変換工程(意図的) 7 / 収集対象設定 2 = 69

捕捉列 = その episode で FAIL したチェックの集合(Dn/Do/DB)。単独捕捉の集計: DB のみ 23 / Dn のみ 16 / Do のみ 0 / 複数同時 30。

注(証拠水準の読み方): 「事後分類」も run 表と promote 履歴に基づく triage 作業の結果であり、無根拠の推定ではない。
「系列」の 4 件は、開発版で先行していた変換コードの実験的変更が原因であることが当時の運用で明らかだったため
個別 triage を省略したもので、原因となった変更自体(#827/#841/#850 等)は確認済み。論文本文の
「上流 59 件は履歴から確認」「自前 10 件は変更まで確認」はこの意味である。

複数原因を含む episode は、公開可否を左右した原因を代表判定とした。優先順位は
一時障害・恒久的品質イベント > コード/設定変更 > 正当な上流変更(例: #41 は A-13(a) を含むが (b)、#58 は上流 churn を含むが変換工程)。
...
| 26 | 2026-06-11 20:19 | cpe(nvd/vulncheck/jvn...) | N | 25.3 | DB | 変換工程(意図的) | A-8系列 開発版先行のCPE関連変更(候補: vuls-data-update #827/#841=06-10，#850=06-18。個別triage無し) | 系列 |
...
| 29 | 2026-06-14 03:57 | cpe(nvd/vulncheck/jvn...) | N | 40.1 | DB | 変換工程(意図的) | A-8系列 開発版先行のCPE関連変更(候補: vuls-data-update #827/#841=06-10，#850=06-18。個別triage無し) | 系列 |
...
| 32 | 2026-06-17 04:00 | cpe(nvd/vulncheck/jvn...) | N | 122.7 | DB | 変換工程(意図的) | A-8系列 開発版先行のCPE関連変更(候補: vuls-data-update #827/#841=06-10，#850=06-18。個別triage無し) | 系列 |
| 33 | 2026-06-17 10:19 | fedora-api | M+N | 72.5 | DB | 上流(a) | A-5 マスアップデート撤回(Bodhi) | 直接 |
| 34 | 2026-06-19 02:36 | fedora-api | M+N | 274.5 | DB | 変換工程(バグ) | A-6 ソート欠落による非決定出力(修正PR) | 直接 |
| 35 | 2026-06-19 10:23 | cpe(nvd/vulncheck/jvn...) | N | 18.6 | DB | 変換工程(意図的) | A-8系列 開発版先行のCPE関連変更(候補: vuls-data-update #827/#841=06-10，#850=06-18。個別triage無し) | 系列 |
| 36 | 2026-06-23 08:51 | amazon | M+N | 7.4 | Dn+Do | 上流(a) | A-7 Amazon 10日分バッチ | 直接 |
| 37 | 2026-06-24 03:29 | cpe(nvd/vulncheck/jvn...) | N | 18.0 | DB | 変換工程(意図的) | A-8 CPE match quality分類 #850 | 直接 |
...
| 58 | 2026-07-31 08:46 | cpe(nvd/vulncheck/jvn...) | M+N | 10.6 | DB+Dn | 変換工程(意図的) | A-23 Fortinet whitelist追加 #901(他target churn(a)を含む) | 直接 |
...
| 62 | 2026-08-06 08:08 | microsoft-cvrf | M+N | 211.3 | DB | 収集対象設定 | A-24 msuc seed一括登録 #211/#216/#217 | 直接 |
| 63 | 2026-08-06 08:32 | cpe(nvd/vulncheck/jvn...) | M+N | 135.5 | DB+Dn | 変換工程(意図的) | A-25 fortinet-cvrf 検知補完 #892 | 直接 |
| 64 | 2026-08-10 12:36 | microsoft-cvrf | M+N | 100.0 | DB | 収集対象設定 | A-26 microsoft-servicing新規ソース追加 | 直接 |

...
- **重要発見**: alma_10 の removed 97 件は baseline リセットの副産物ではなく、**上流 advisory の「骨抜き」の顕在化**。例: **ALSA-2026:2721**(kernel security update)がピン時点 73 criteria(`< 0:6.12.0-124.38.1.el10_1`)→ 現行の `errata.full.json` では kernel-doc / kernel-abi-stablelists の 2 criteria のみに縮退。生存 136 advisory 中 79 件(58%)がパッケージ数半減以下。`errata.full.json` / OSV / advisory HTML の三者一致でこれが「上流の姿」と確認。CVE 例: CVE-2023-53034 の検知が消失。AlmaLinux Mantis #644 への追加証拠として記録
